...
| 3.13 | 工程基建：性能基准套件 + 预期 DSL 快照 + mmdc 语法校验 + CI 跨平台矩阵 | `benchmark/benchmark_test.go` | 🔄 | 性能基准套件已建立（bench-10k/50k/100k）；预期 DSL 快照和 mmdc/CI 待实现 (G3部分完成, G5-G7仍待) |
...
| PDF 导出含中文字符 | ❌ | 未实现 |
| 基准测试套件 (bench-10k/50k/100k) | ✅ | `benchmark/benchmark_test.go` 覆盖 E2E/解析/图构建/Wiki/PageRank |
| 增量索引：新增文件 | ❌ | 未实现 (G4) |
| 增量索引：删除文件清理 | ❌ | 未实现 (G4) |
| Prompt 快照回归 | ❌ | 未实现 (G2) |
| 图表 DSL 预期快照 | ❌ | 未实现 (G5) |
