...
| 3.9 | 大规模仓库性能优化 | 无 | ❌ | 无缓存策略、无并发控制调优 |
| 3.10 | 提示词优化（按语言） | `internal/docgen/docgen.go` | ⚠️ | prompt 已统一中文；无按语言（Python/Go/JS）定制的模板 |
| 3.11 | Rust / C++ AST 支持 | 无 | ❌ | 未实现 |
...
| G2 | 无 Prompt 快照/回归目录 (`llm-responses/`) | M2-M3 LLM 输出稳定性 | M3 文档深度阶段 | 1.2, 3.1 |
| G3 | 无性能基准套件 (`benchmark/bench-*`) | M3 性能优化无度量 | M3 性能阶段 | 3.9 |
| G4 | 向量存储无增量索引，每次全量重建 | M2 RAG 大项目体验 | M3 性能阶段 | 2.2 |
| G5 | 无 `expected/` 目录存储预期图表 DSL | M1 图表回归测试 | 低优先级 | 1.3 |
| G6 | Mermaid 语法无 `mmdc` 自动化校验 | M1 图表正确性 | 低优先级 | 1.3 |
| G7 | 无 CI/CD 跨平台矩阵 | M3 发布门禁 | M3 发布前 | 发布门禁 |
